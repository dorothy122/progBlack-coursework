...
window.addEventListener('DOMContentLoaded', function(event){
  ...
  this.document.getElementById("content").addEventListener("click", (event) => {
    if (event.target.tagName == "IMG") {
      ...
      .then(data => {
        // generate card and add to popover
        let popover = bootstrap.Popover.getInstance(image)
        popover.setContent({
          ".popover-body": enlarge(data)
        })
...
function enlarge(data) {
  ...
  // change the content of that images popover to the card
  //let popover = bootstrap.Popover.getInstance(image)
  ...
  //image.setAttribute("data-bs-content", card)
...
function displayImage(data) {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    // make into a bootstrap popover
    // generated by ChatGPT given promt: "Can I, and how do I edit bootstrap popovers so that a popover would appear when I click an image. The image is generated in client side javascript."
    img.tabIndex = 0;
    img.setAttribute("role", "button")
    img.style.cursor = "pointer"
    img.setAttribute("data-bs-toggle", "popover")
    img.setAttribute("data-bs-trigger", "focus")
    img.setAttribute("data-bs-content", "Loading...")
